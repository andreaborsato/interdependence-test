...
function setup() {
    ...
    finalColorArray = Array.from({length: newSize}, (_, i) => myColorArray[i % myColorArray.length]); // creo array da myColorArray
    _nsRate = 0.00047;
    _maxPoint = 20;
    capArray = new Array({length: _objectNum}); // array per stile cap

    for (let i = 0; i < _objectNum; i++) { // creo numero bubbles basate su classe line e le metto dentro array _aryObject
      capStyle = ROUND;
...
function draw() {
    ...
    for (let j =  19; j >=  0; j--) { // calcolare distanza per evitare effetto scia
      if (j ===  0) {
          // caso speciale per primo elemento 
          continue;
        }
        d = Math.floor(Math.sqrt((point[j][0] - point[j -  1][0]) **  2 + (point[j][1] - point[j -  1][1]) **  2));
        totalDistance += d;
...
class mouseLine { // classe per bubble puntatore mouse
...
class line { // classe per tutte le altre bubble
